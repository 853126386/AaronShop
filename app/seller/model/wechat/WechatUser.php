...
use app\seller\model\user\{User, UserBill, UserExtract};
use app\seller\model\order\{StoreOrder, StoreOrderStatus};
...
class WechatUser extends BaseModel
{
    ...
    public static function agentSystemPage($where = [])
    {
        ...
        $orderSql = StoreOrder::alias('o')
            ->join('user u', '(u.spread_uid = o.uid) || (u.uid = o.uid)')
            ->where(['paid' => 1, 'refund_status' => 0])
            ->group('o.uid')
            ->field(['sum(pay_price) as order_price', 'count(id) as order_count', 'o.uid as ouid'])
            ->fetchSql(true)
            ->select();
